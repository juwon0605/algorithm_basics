...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++) {
			cin >> num;
			if (num == 1) houseV.push_back(make_pair(i, j));
			else if (num == 2) pizzaV.push_back(make_pair(i, j));
		}
	}
	DFS(0, 0);
	cout << res;
...
void DFS(int start, int L) {
	if (L == m) {
		...
		for (int i = 0; i < houseV.size(); i++) {
			int ii = houseV[i].first;
			int jj = houseV[i].second;
			int minDis = 2147000000;
			for (int j = 0; j < m; j++) {
				int dis = abs(ii-pizzaV[choice[j]].first) + abs(jj-pizzaV[choice[j]].second);
		...
		for (int i = start; i < pizzaV.size(); i++) {
			choice[L] = i;
			DFS(i + 1, L + 1);
		}
	}
